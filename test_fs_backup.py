...
def load_set_support(model, dataset):
    ...
    with torch.no_grad():
        for subset in range(cfg.run_num):
            ...
            support_vector = {}
            support_set = dataset.support_set[subset]   # 每个run_num对应一个support_sets
            for cls in dataset.SEMANTIC_LABELS:
                sup_vectors = []
                list_scenes = support_set[cls]          # 属于cls类别的support scenes (k-shot)
                for i in range(cfg.k_shot):
                    ...
                    sup_vec = model.process_support(support_dict, training=False)   # 直接拿到这个场景的support feature（经过了masked average pooling）
                    sup_vectors.append(sup_vec)
                sup_vectors = torch.cat(sup_vectors, dim=0)
                mean_vector = torch.mean(sup_vectors, dim=0)    # 对k个support feature取平均值得到最终的support feature
                support_vector[cls] = mean_vector.cpu()
            set_support_vectors.append(support_vector)  # 每个run中对每个的类别有一个平均的support feature
        ...
        if cfg.eval:
            ...
            for k in range(cfg.run_num):
                ...
                for i in range(len(pred_info_arr[k])):  # 就是总共的测试集长度
                    pred_info = pred_info_arr[k][i]     # 第k轮种针对第i个query scene生成的prediction
