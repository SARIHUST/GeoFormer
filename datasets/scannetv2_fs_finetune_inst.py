...
class FSInstDataset:
    ...
    def trainMergeFS(self, ids):
        ...
        support_pc_maxs = []
        support_labels = []
        support_instance_labels = []
        support_instance_pointnum = []
        support_total_inst_num = 0
        ...
        for idx, id in enumerate(ids):  # ids中的索引无效，需要确定sampled_class之后重新选择scene
            ...
            while True:
                ...
                support_label = support_label == sampled_class
                support_instance_label[(support_label == 0).nonzero()] = -100
                support_instance_label = self.getCroppedInstLabel(support_instance_label, valid_idxs=None)
                support_inst_num, support_inst_infos = self.getInstanceInfo(support_xyz_middle, support_instance_label.astype(np.int32))
                support_inst_pointnum = support_inst_infos["instance_pointnum"]
                support_instance_label[np.where(support_instance_label != -100)] += support_total_inst_num
                support_total_inst_num += support_inst_num
            ...
            support_labels.append(torch.from_numpy(support_label))
            support_instance_labels.append(torch.from_numpy(support_instance_label))
            support_instance_pointnum.extend(support_inst_pointnum)
        ...
        support_labels = torch.cat(support_labels, 0).long()
        support_instance_labels = torch.cat(support_instance_labels, 0).long()
        support_instance_pointnum = torch.tensor(
            support_instance_pointnum, dtype=torch.int
        )
    ...
    def testMergeFS(self, ids):
        ...
        if cfg.fix_support:
            ...
            for l in test_comb["active_label"]:
                ...
                support_label = support_label == l
                support_instance_label[(support_label == 0).nonzero()] = -100
                support_instance_label = self.getCroppedInstLabel(support_instance_label, valid_idxs=None)
                
                support_label = torch.from_numpy(support_label)
                support_instance_label = torch.from_numpy(support_instance_label)
                ...
                support_dict = {
                    ...
                    "instance_labels": support_instance_label,
                }
